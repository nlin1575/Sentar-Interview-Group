...
exports.isOpenAIAvailable = void 0;
...
async function generateEmbedding(text) {
    ...
    try {
        ...
        if (result.error)
            throw result.error;
        if (result.status !== 0)
            throw new Error(result.stderr || 'embed.py failed');
        ...
        return {
            ...
            tokens,
            cost: (0, mockEmbedding_1.calculateEmbeddingCost)(text),
            type: 'mock'
        };
    }
}
...
`;
    ...
    if (profile.entry_count === 0) {
        ...
        if (emotion_flip)
            ctx += ' Emotion flip detected.';
        if (carry_in)
            ctx += ' Continues recent pattern.';
    }
    return examples + ctx + '\nReply ≤120 characters (one sentence, encouragement required):';
}
/*──────────────── mock fallback (unchanged logic) ────────────────*/
function generateMockGPTResponse(parsed, profile, carry_in, emotion_flip) {
...
function generateEmpathicResponseMock(parsed, profile, carry_in, emotion_flip) {
    const vibe = parsed.vibe[0] || 'neutral';
    const theme = parsed.theme[0] || 'general';
    if (profile.entry_count === 0)
        return firstEntry(vibe);
    if (emotion_flip)
        return flip(vibe);
    if (carry_in)
        return carry(vibe, theme);
