...
function App() {
  ...
  const [bulkResults, setBulkResults] = useState<BulkProcessResponse | null>(null);
      ...
      <main className="main">
        ...
        <div className="csv-upload-section">
          ...
          <div className="csv-upload-form">
            ...
            >
              {bulkLoading ? '🔄 Processing CSV...' : '🚀 Process CSV Entries'}
            </button>
          </div>
        </div>
            ...
            <div className="final-profile">
              ...
              <div className="profile-stats">
                <div className="stat">
                  ...
                  <span className="value">{bulkResults.final_profile.entry_count}</span>
                </div>
                <div className="stat">
                  <span className="label">Dominant Vibe:</span>
                  <span className="value">{bulkResults.final_profile.dominant_vibe}</span>
                </div>
                <div className="stat">
                  <span className="label">Top Themes:</span>
                  <span className="value">{bulkResults.final_profile.top_themes.join(', ')}</span>
                </div>
                <div className="stat">
                  <span className="label">Traits:</span>
                  <span className="value">{bulkResults.final_profile.trait_pool.join(', ')}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bulk-entries">
              ...
              <div className="entries-list">
                {bulkResults.results.map((result, index) => (
                  <div key={index} className={`entry-result ${result.error ? 'error' : 'success'}`}>
                    <div className="entry-header">
                      <span className="entry-number">#{result.index}</span>
                      <span className="entry-status">
                        {result.error ? '❌ Failed' : '✅ Success'}
                      </span>
                    </div>
                    <div className="entry-transcript">
                      "{result.transcript.substring(0, 100)}{result.transcript.length > 100 ? '...' : ''}"
                    </div>
                    {result.error ? (
                      <div className="entry-error">Error: {result.error}</div>
                    ) : (
                      <div className="entry-details">
                        <div className="entry-response">Response: "{result.response_text}"</div>
